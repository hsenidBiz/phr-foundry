---
name: phx-write-frd
description: Use when creating, editing, converting, reviewing, or publishing a PeoplesHR Feature Requirements Document (FRD) — the business/functional wiki document in the PHR-X project wiki. Triggers on "write an FRD for", "draft the feature requirements doc", "add a section to the FRD", "does this FRD follow our standard", "convert this doc to our FRD format", an FRD- document ID, a link to an FRD page in the PeoplesHR wiki, or a request to publish an FRD. Also triggers at the end of a design, grilling, or requirements conversation when the user asks to write it up, capture it, or turn it into an FRD — the session transcript and any ADRs or CONTEXT.md it produced are the input. The template and sample live in the wiki, so this skill needs the Azure DevOps MCP server and stops without it. For the technical companion document, use phx-write-sdd in the dev-kit plugin instead.
---

# PHX Write FRD

Authors the **FRD — Feature Requirements Document**: the business/functional-facing document for a
PeoplesHR feature. Drafted by the Business Analyst, accountable to the Product Owner, consulted by
the Solutioning Engineer. ID prefix `FRD-`.

Its technical companion — the Architecture / Solution Design Document (`ARCH-`) — is **not** this
skill's job. If asked for that, use `phx-write-sdd`, which ships in the **`dev-kit`** plugin
(`/dev-kit:phx-write-sdd`). If `dev-kit` is not installed, say so rather than writing the SDD here.
...
DevOps MCP server, **every run**, before producing or judging any FRD:

| Purpose | Wiki page | Path |
|---|---|---|
| **Template** — the standard: section list, numbering, wording | `10165/Feature-Requirement-Document-(FRD)` | `/Manifesto/Document-Templates/Engineering/Feature-Requirement-Document-(FRD)` |
| **Sample** — the output shape of a real, filled document | `10169/Sample` | `/Manifesto/Document-Templates/Engineering/Feature-Requirement-Document-(FRD)/Sample` |

Read them with the ADO MCP wiki tool's `get_page_content` action, passing the full URL:

```
action: get_page_content
url: https://dev.azure.com/PeoplesHR/PHR-X/_wiki/wikis/PHR-X.wiki/10165/Feature-Requirement-Document-(FRD)
url: https://dev.azure.com/PeoplesHR/PHR-X/_wiki/wikis/PHR-X.wiki/10169/Sample
```

If the URL form is rejected, fall back to `wikiIdentifier: PHR-X.wiki`, `project: PHR-X`, and the
path above. **Do not reconstruct the section list from memory, from this file, or from the SDD
...
- **That you could not read the FRD template and sample from the PeoplesHR Azure DevOps wiki**, named
...
- **That no FRD will be produced as a result**, and why: the section list, numbering and wording come
...
- substitute the SDD template, an older PRD template, or a generic requirements outline;
...
the developer or copied from a source they named (a work item, a repository file, a linked SDD).
...
Never invent: names, roles, dates, version numbers, document IDs, metrics or targets, personas,
module names, priorities, acceptance criteria, or approval decisions.

**Open Questions & Risks is not a gap dump.** That table records decisions the team genuinely has
not made yet — an unresolved business trade-off, a ruling owed by Finance. It is not where inputs
you were never given go. Missing Document Control fields, absent success metrics, unwritten
acceptance criteria and unsupplied NFRs are collection failures, and they belong in the question you
...
is invented content, whatever it is wearing — and FRD acceptance criteria become TDD test cases, so
a target nobody agreed to gets built against. The only marker the standard has is `N/A` plus a
one-line reason, and only in sections the template marks *(if applicable)*.

| Rationalization | Reality |
|---|---|
| "The board can correct the details live" | They will not. Written numbers get read as agreed and built against. |
...
| "A draft with visible seams is better than no document" | Not for this document type. Stop and ask. |
...
same chat and now says "turn that into an FRD."

**The conversation is a legitimate source.** What the developer decided in this session is supplied
input, exactly like a work item or a repository file. Writing it into the document is not invention.
So are the artifacts the session produced — a `CONTEXT.md` glossary entry, an ADR under `docs/adr/`,
a file a sub-agent read and reported. Prefer the artifact's wording over your memory of the chat.
...
recommendation, left unanswered when the session moved on, is still your guess — promoting it into
the FRD launders a suggestion into a requirement, and FRD acceptance criteria become TDD tests. If
you cannot point to the developer settling it, it is a gap.

Same for the shape of the discussion: a session that produced sharp acceptance criteria for two user
stories has given you §6, not §2's success metrics and not §10's NFRs. Depth in one area is not
permission to fill the rest.

**A session almost never supplies Document Control.** The FRD ID, feature name, BA and Product Owner
names, version, roadmap/epic reference, linked SDD, and the target wiki path are metadata nobody
states while designing. Expect to ask for all of them.

**So the first move is a coverage pass, not a draft.** Read the wiki template, map the session onto
its sections, and come back with one consolidated question listing every section the session did not
settle — by number. Then wait. Do not draft the covered sections and ask about the rest afterwards:
a half-written FRD in the chat creates pressure to keep the invented half.

## Route by what is being asked

| Situation | What to do |
|---|---|
| Creating a new FRD | Follow the wiki template exactly. Not optional. |
...
one line ("this page follows the FRD template" / "this page is not in our format — I'll edit in
...
2. **Strip the scaffolding:** the `*(required)*` / `*(if applicable)*` markers from headings, the
   "How to use this template" blockquote, and each section's italic guidance text once filled. The
   §9 Modular Monolith context blockquote and its filling guidance are guidance too — they go once
   the section is written.
3. **H1 is the document type, verbatim from the sample** — `# PeoplesHR — Feature Requirements
   Document (FRD)`. Not the feature name, not a title of your own. The feature name goes in Document
   Control. Keep the italic role/companion line beneath it.
4. **Never carry the sample banner.** The `> **SAMPLE / ILLUSTRATIVE ONLY**` blockquote belongs to
   the sample alone.
5. **Repeat blocks** — `FR-1`, `FR-2`, … — numbered sequentially, one per requirement, each with
   MoSCoW priority and Given/When/Then acceptance criteria.
...
9. **Definition of Ready checklist:** leave boxes unticked unless the developer states the gate was
   passed. Ticking a box nobody passed is invention.

## Stay on the business side of the line

§9 (Cross-Module & Integration Impact) records the *business need* for each cross-module touchpoint
and deliberately leaves the mechanism open. Direct DB access vs. internal API vs. ADAB is the SDD's
call — do not prescribe it here. Likewise §8 is data in business terms; table and column names
belong in the SDD, not the FRD.

Every §9 touchpoint you write becomes a row the SDD must resolve, so write only the touchpoints the
developer actually described.
...
**Follow the repo's existing convention first.** Before choosing a path, look for FRDs already in the
repo (`docs/frd/`, `docs/requirements/`, alongside `docs/adr/`, or wherever they actually live). If
one exists, match it — its folder and its filename shape. Only when there is no precedent, use:

```
docs/frd/<FRD-ID>-<kebab-case-feature-name>.md
```

e.g. `docs/frd/FRD-EI-2026-014-employee-information-360-snapshot.md`. Both parts come from Document
...
- **Document ID** comes from the developer. Shape is `FRD-<MODULE>-<YYYY>-<NNN>` (e.g.
  `FRD-EI-2026-014`). Validate the shape; if it is missing or malformed, ask. Never allocate the next
  number yourself, and never derive an FRD's ID from a linked SDD without the developer confirming
  it.
...
[`FRD Template`](/Manifesto/Document%2DTemplates/Engineering/Feature%2DRequirement%2DDocument%2D\(FRD\))
```
...
| 4 | The document has no unresolved gaps | Stop and ask. Never publish a document with holes in it. |
...
> **`phx-write-frd` needs the Azure DevOps MCP server**
>
> The FRD template and sample live in the PHR-X wiki, and every wiki read and write goes through it —
...
- About to write a name, date, ID, metric, persona, or acceptance criterion the developer did not give you
...
- About to promote your own unanswered recommendation from a grilling round into the document as a settled requirement
...
- About to prescribe a technical mechanism (direct DB / internal API / ADAB) or a table name in §9 or §8
- About to file inputs you were never given as "Open Questions" and deliver anyway
- About to tick a Definition of Ready box nobody confirmed
