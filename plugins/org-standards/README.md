...
`/org-standards:hrm-configuration-document` — or let
...
`hrm-configuration-document` when a finished CR needs its Configuration Document.
...
- **Versioned by semver** in `plugin.json` (currently `2.11.1`); bump it on each
