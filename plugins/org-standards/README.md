...
`/org-standards:hrm-configuration-document`, `/org-standards:phx-product-context`
— or let Claude load it automatically:
...
sent when something happens in a module, `hrm-configuration-document` when a
finished CR needs its Configuration Document, `phx-product-context` whenever a
question turns on how a PeoplesHR module actually behaves.
...
- **Versioned by semver** in `plugin.json` (currently `2.12.0`); bump it on each
