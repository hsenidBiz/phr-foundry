...
│   │   ├── .claude-plugin/
│   │   │   └── plugin.json       # Manifest (version: 2.12.0 — see Versioning); also
...
The plugin ships six skills of its own, plus two MCP servers.
...
is at `2.12.0`, `ba-kit` at `1.0.0`). Claude Code resolves a plugin's version from
